...
namespace gl {
    ...
    inline void uniform(const char* name, std::floating_point auto value) {
        glUniform1f(_::get_uniform_location(name), value);
    }

    inline void uniform(const char* name, int value) {
        glUniform1i(_::get_uniform_location(name), value);
    }

    inline void uniform(const char* name, unsigned int value) {
        glUniform1ui(_::get_uniform_location(name), value);
    }

    inline void uniform(const char* name, bool value) {
        glUniform1i(_::get_uniform_location(name), value);
    }

    inline void uniform(const char* name, const Vector3& value) {
        glUniform3f(_::get_uniform_location(name), value.x, value.y, value.z);
    }

    inline void uniform(const char* name, const Quaternion& value) {
        glUniform4f(_::get_uniform_location(name), value.v.x, value.v.y, value.v.z, value.s);
    }

    inline void uniform(const char* name, const MatrixS<4, 4>& value) {
